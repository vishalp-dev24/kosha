    ...
    <MarketingShell>
      <main className="mx-auto max-w-7xl px-4 py-20 md:px-6 md:py-28">
        <div className="max-w-4xl">
          ...
          <h1 className="mt-6 font-display text-6xl leading-[0.9] tracking-[-0.04em] md:text-8xl">For teams whose documents carry risk.</h1>
        ...
        <MotionSection className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {useCases.map((useCase) => {
            ...
            return (
              <article key={useCase.title} className="group rounded-2xl border border-line bg-white/62 p-6 transition hover:border-blue/35 hover:shadow-ledger">
                <Icon className="h-6 w-6 text-blue" aria-hidden="true" />
                <h2 className="mt-6 text-xl font-semibold text-ink">{useCase.title}</h2>
                <p className="mt-4 text-sm leading-relaxed text-ink/60">{useCase.copy}</p>
                <div className="mt-6 rounded-xl border border-line/60 bg-paper/72 p-4 text-sm font-semibold text-ink/72">
        ...
        <MotionSection className="mt-16 md:mt-20 rounded-3xl bg-ink p-8 text-paper md:p-12">
