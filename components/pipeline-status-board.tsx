  ...
  return (
    <div className="rounded-[1.75rem] border border-paper/10 bg-paper/7 p-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="text-xs font-bold uppercase tracking-[0.16em] text-paper/42">Pipeline status board</div>
          <h2 className="mt-2 font-display text-4xl tracking-[-0.07em] text-paper">Ready is earned, not assumed.</h2>
        </div>
        <div className="rounded-full border border-teal/25 bg-teal/10 px-4 py-2 text-sm font-semibold text-teal">production gate active</div>
      </div>
        ...
        <div className="grid min-w-[840px] grid-cols-6 gap-3">
          {pipelineStatus.map((item, index) => (
            <motion.div
              ...
              className="rounded-2xl border border-paper/10 bg-ink p-4"
            >
              <div className="font-mono text-xs font-bold uppercase tracking-[0.13em] text-paper/48">{item.label}</div>
